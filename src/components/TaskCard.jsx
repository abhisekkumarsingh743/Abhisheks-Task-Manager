...
export default function TaskCard({ task }) {
  return (
    <div className="bg-white p-3.5 rounded-xl border border-slate-200 task-card-shadow hover:border-blue-400 transition-all cursor-pointer group">
      <h3 className="font-bold text-xs text-slate-800 mb-2.5 group-hover:text-blue-600 transition-colors leading-tight uppercase">
        {task.title}
      </h3>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-2 text-[9px] text-slate-500">
          ...
          <span className="font-black text-slate-400 uppercase">Owner:</span>
          <span className="text-slate-700 font-bold">{task.assignee}</span>
        </div>
        <div className="flex items-center gap-2 text-[9px] text-slate-500">
          <Clock size={11} className="text-slate-400" /> 
          <span className="font-black text-slate-400 uppercase">Schedule:</span>
          <span className="text-slate-700 font-bold">{task.time || "N/A"}</span>
        </div>
        <div className="flex items-center gap-2 text-[9px] text-slate-500">
          <Calendar size={11} className="text-slate-400" /> 
          <span className="font-black text-slate-400 uppercase">Deadline:</span>
